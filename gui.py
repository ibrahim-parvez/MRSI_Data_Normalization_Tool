...
from PyQt6.QtGui import QFont, QIcon, QCursor, QPainter, QColor, QPen, QAction, QKeySequence, QShortcut
from PyQt6.QtWidgets import (
    ...
    QToolTip, QStyleOptionGroupBox
)
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QTimer, QPoint, QRect, QSize, QPropertyAnimation, QEasingCurve
...
# --- Import Tab UIs ---
# Ensure these files exist in your directory or comment them out if testing without them
try:
    from carbonate_tab import CarbonateTab
    from water_tab import WaterTab
    from combine_tab import CombineTab
except ImportError:
    # Fallback mock classes if files are missing during testing
    class CarbonateTab(QWidget): 
        def get_run_parameters(self): return {}, "", ""
...
class SlopeGroupWidget(QFrame):
    """
    Widget representing one 'Instance' of slope calculation.
    Uses FlowLayout for responsive wrapping.
    """
    def __init__(self, index, available_materials, selected_materials, parent_widget):
        super().__init__()
        self.parent_widget = parent_widget 
        self.setFrameShape(QFrame.Shape.StyledPanel)
        self.setStyleSheet("QFrame { background-color: #f9f9f9; border: 1px solid #ddd; border-radius: 4px; }")
        
        # Use VBox for the outer frame
        self.layout = QVBoxLayout(self)
        self.layout.setContentsMargins(5, 5, 5, 5)
        
        # Header Row: "Instance X" + Delete Button
        ...
        self.del_btn = QPushButton("−") # Using Math Minus
        self.del_btn.setFixedSize(24, 24)
        self.del_btn.setToolTip("Remove this group") # Hover text
        self.del_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        
        # FIXED STYLE: Removed padding that was hiding the text
        self.del_btn.setStyleSheet("""
            QPushButton {
                ...
                border-radius: 4px; 
                font-weight: bold;
                font-size: 16px; /* Fits comfortably in 24px */
                padding: 0px;    /* Crucial: prevents text clipping */
            }
            QPushButton:hover {
        ...
        # Container for checkboxes using FlowLayout
...
class MaterialTypeWidget(QWidget):
    ...
    def _create_table_section(self):
        ...
        # --- VISUAL UPGRADES ---
        # 1. Enable alternating colors for readability
        self.table.setAlternatingRowColors(True)

        # 2. CSS to highlight Headers (Row 1 / Col 1) and clean up lines
        self.table.setStyleSheet("""
            QTableWidget {
                gridline-color: #cccccc;
                background-color: white;
                alternate-background-color: #f5f5f5; /* Light gray for data rows */
            }
            /* This styles BOTH the top row and the left column headers */
            QHeaderView::section {
                background-color: #e0e0e0; /* Gray Highlight */
                color: #333;
                padding: 4px;
                border: 1px solid #bfbfbf;
                font-weight: bold;
            }
            /* The top-left corner button */
        ...
        # --- FIXED WIDTH LOGIC: STRICT STRETCH ---
        header = self.table.horizontalHeader()
        
        # 1. Force everything to fit in the window (Stretch mode)
        for i in range(7):
            header.setSectionResizeMode(i, QHeaderView.ResizeMode.Stretch)
            
        # 2. Allow headers to wrap text so long headers don't force wide columns
        header.setDefaultAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
        header.setMinimumSectionSize(30) # Allow shrinking very small
        
        # 3. Give Name (col 0) slightly more weight (Stretch Factor 2 vs 1)
        ...
        # Optional: Make buttons slightly taller/cleaner
        self.add_btn.setStyleSheet("padding: 5px;")
        self.remove_btn.setStyleSheet("padding: 5px;")
    ...
    def _insert_table_row(self, mat_data=None):
        ...
        # Define highlight color (Light Gray)
        ...
        for i, key in enumerate(keys):
            ...
            # --- HIGHLIGHT LOGIC: First Row OR First Column ---
            if i == 0:
                item.setBackground(highlight_color)
                item.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold)) # Optional: Make text bold
...
class AdvancedSettingsTab(QWidget):
    ...
    def _create_general_config(self):
        ...
        self.input_stdev.setFixedWidth(60) 
    ...
    def _create_calc_logic_section(self):
        ...
        row2.addWidget(create_info_label(
            ...
            "<ul>"
            "<li><b>All Values:</b> Keeps every measurement, even potential errors.</li>"
            "<li><b>Outliers Excluded:</b> Removes data points flagged as outliers from the final report.</li>"
            "</ul>"
        ))
        row2.addWidget(QLabel(":"))
...
class DataToolApp(QWidget):
    ...
    def on_tab_changed(self, index):
        ...
        # 4. Handle Button Visibility (Always show unless running)
    ...
    def show_menu(self):
        ...
        # FIX: Calculate position relative to the BUTTON, not the main window
        # (0, height) is the bottom-left corner of the button
